...
import {
  convertFahrenheitTempsToCelsius,
  convertCelsiusTempsToFahrenheit,
} from "../utils";
...
const WeatherForecast = () => {
  ...
  const handleChange = (event) => {
    ...
    if (showChart) {
      // only convert when a chart is displayed
  ...
  return (
    <React.Fragment>
      {Object.keys(weather.weather).length === 0 ? (
        <Loading />
      ) : (
        <Container className="cover">
          <div>
            <DegreeToggle handleChange={handleChange} value={scale} />
          </div>
          <div>
            <UserPagination paginate={paginate} pagination={pagination} />
          </div>
          <div className="data">
            {Object.keys(weather.weather).map(function (key, index) {
              let temps = weather.weather[key].map((temp) => temp.main.temp);
              return pagination.includes(key) ? (
                <div key={index}>
                  <DayCard
                    date={key}
                    ...
                    scale={scale}
                    temps={temps}
                    showChartVisibility={showChartVisibility}
          ...
          {showChart ? (
            <BarChart temperatures={temperatures} scale={scale} />
          ) : (
            ""
          )}
        </Container>
